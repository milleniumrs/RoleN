//! `maestro` — headless CLI + TUI launcher (Requirements FR-11).
...
#[command(
    name = "maestro",
    version,
    about = "Maestro — a conductor for LLM-powered development"
)]
struct Cli {
    #[command(subcommand)]
...
enum Commands {
    ...
    /// Manage LLM providers (Requirements FR-1)
    Provider {
        #[command(subcommand)]
        action: ProviderAction,
    },
    /// Show token usage / quota information (Requirements FR-4)
    Quota {
    ...
    /// Manage routing rules (Requirements FR-3) — canonical YAML at rules.yaml
    Rule {
        #[command(subcommand)]
        action: RuleAction,
    },
    /// Run an agent task headless through the built-in runtime (Requirements FR-12)
    Run {
    ...
    /// Run a batch of tasks as a parallel DAG through the orchestrator (Requirements FR-7/FR-8)
    Batch {
    ...
    /// Manage projects: scaffold, interview, Requirements/AGENTS.md/skills (Requirements FR-5/FR-6)
    Project {
        #[command(subcommand)]
    ...
    /// Run a task through a wrapped CLI agent in a PTY (Requirements FR-13)
    Cli {
        #[command(subcommand)]
        action: CliAction,
    },
...
enum ProjectAction {
    ...
    /// Generate REQUIREMENTS.md/REQUIREMENTS.json, AGENTS.md, skill suggestions and tasks.yaml
    Build {
        #[arg(long)]
        name: String,
    },
...
fn main() -> Result<()> {
    let cli = Cli::parse();
    match cli.command {
        None | Some(Commands::Tui) => maestro_tui::run()?,
        ...
        Some(Commands::Gui) => {
            maestro_gui::run().map_err(|e| anyhow::anyhow!("could not start the GUI: {e}"))?
        }
        ...
        Some(Commands::Prd { validate }) => {
            let problems =
                maestro_core::project::validate_prd_json(std::path::Path::new(&validate))
                    .map_err(|e| anyhow::anyhow!(e))?;
            if problems.is_empty() {
                println!(
                    "{validate}: valid REQUIREMENTS.json (schema v{})",
                    maestro_core::project::Requirements_JSON_SCHEMA
                );
            } else {
                for p in &problems {
                    println!("✗ {p}");
...
fn cli_cmd(action: CliAction) -> Result<()> {
    match action {
        CliAction::Run {
            ...
            let report = maestro_cliadapters::run_cli_session(
                &p,
                &task,
                &workdir,
                None,
                &mut |ev| match ev {
                    maestro_cliadapters::CliEvent::Output(_) => {
...
fn workspace_root() -> Result<std::path::PathBuf> {
    let (cfg, _) = maestro_core::config::Config::ensure()?;
    cfg.ensure_workspace_root()?;
    Ok(cfg.general.workspace_root)
}

fn question_mode(arg: Option<String>) -> Result<maestro_core::types::QuestionMode> {
    use maestro_core::types::QuestionMode;
    match arg.as_deref() {
        None => Ok(maestro_core::config::Config::load()
...
fn run_interview(
    meta: &mut maestro_core::project::ProjectMeta,
    mode: maestro_core::types::QuestionMode,
) -> Result<usize> {
    use maestro_core::types::{Clarification, ClarificationStatus};
...
fn project_cmd(action: ProjectAction) -> Result<()> {
    use maestro_core::project as proj;
    match action {
        ProjectAction::New {
            name,
            description,
            ...
            println!("next: maestro project build --name {}", meta.id);
        }
        ProjectAction::List => {
            let root = workspace_root()?;
            ...
            if projects.is_empty() {
                println!(
                    "no projects in {} — create one with `maestro project new --name …`",
                    root.display()
                );
                return Ok(());
            }
        ...
        ProjectAction::Build { name } => {
            ...
            if !skills.is_empty() {
                ...
                println!(
                    "  install with: maestro project skills --name {} --install <skill>",
                    meta.id
                );
            }
            ...
            match maestro_orchestrator::daggen::generate_dag(&meta, &prd) {
                Ok(tasks) => {
                    ...
                    println!(
                        "  maestro batch --spec {} --workdir {}",
                        dir.join("tasks.yaml").display(),
                        dir.display()
                    );
                }
                Err(e) => println!(
                    "⚠ DAG proposal failed ({e}); retry `maestro project build --name {}`",
                    meta.id
                ),
            }
...
fn batch_cmd(
    ...
    let spec = maestro_orchestrator::BatchSpec::load(std::path::Path::new(&spec_path))?;
    if !watch {
        println!("batch: {} task(s), workdir {workdir}", spec.tasks.len());
    }
    let opts = maestro_orchestrator::BatchOptions {
    ...
    let report = maestro_orchestrator::run_batch(&spec, &opts, &mut |ev| {
        if watch {
            // FR-11.4: NDJSON event stream for external tooling
            let line = match &ev {
                maestro_orchestrator::BatchEvent::TaskStarted { id, role } => {
                    serde_json::json!({"event": "task_started", "id": id, "role": role})
                }
                maestro_orchestrator::BatchEvent::Waiting { id, reason } => {
                    serde_json::json!({"event": "waiting", "id": id, "reason": reason})
                }
                maestro_orchestrator::BatchEvent::TaskDone { id, tokens, steps } => {
                    serde_json::json!({"event": "task_done", "id": id, "tokens": tokens, "steps": steps})
                }
                maestro_orchestrator::BatchEvent::TaskFailed { id, error } => {
                    serde_json::json!({"event": "task_failed", "id": id, "error": error})
                }
                maestro_orchestrator::BatchEvent::Agent { id, line } => {
                    serde_json::json!({"event": "agent", "id": id, "line": line})
                }
                maestro_orchestrator::BatchEvent::AllDone { done, failed } => {
            ...
            match ev {
                ...
                maestro_orchestrator::BatchEvent::Agent { id, line } => println!("[{id}] {line}"),
                maestro_orchestrator::BatchEvent::AllDone { done, failed } => {
...
fn rule_cmd(action: RuleAction) -> Result<()> {
    use maestro_core::rules::RuleSet;
    use maestro_core::types::Rule;
    match action {
        RuleAction::List => {
            let rules = RuleSet::load()?;
            if rules.rules.is_empty() {
                println!("no rules — seed defaults with `maestro rule init`");
                return Ok(());
            }
        ...
        RuleAction::Init => {
            ...
            println!(
                ...
                maestro_core::config::rules_file()?.display()
            );
        }
        RuleAction::Add {
            role,
...
fn run_cmd(
    ...
    let mut opts = maestro_runtime::AgentOptions {
        workdir: workdir.into(),
        role,
        task,
    ...
    let report = maestro_runtime::agent::run(&mut opts, &mut |ev| {
        use maestro_runtime::AgentEvent::*;
        let line = match ev {
            Routed {
                provider,
                model,
...
fn provider_cmd(action: ProviderAction) -> Result<()> {
    match action {
        ProviderAction::List => {
            let reg = providers::ProviderRegistry::load()?;
            if reg.is_empty() {
                println!("no providers registered — try `maestro provider detect --register` or `maestro provider add --help`");
                return Ok(());
            }
            println!(
                "{:<18} {:<14} {:<40} {:<7} KEY",
        ...
        ProviderAction::Add {
            ...
            let tunnel = match &tunnel {
                Some(spec) => {
                    let mut t = maestro_core::types::TunnelSpec::parse(spec)
                        .map_err(|e| anyhow::anyhow!(e))?;
            ...
            let mut auth = maestro_core::types::AuthKind::Key;
            let oauth_key_ref = if let Some(path) = &oauth_import {
                ...
                auth = maestro_core::types::AuthKind::OAuth;
            ...
            let endpoint = endpoint.or(match ptype {
                ...
                ProviderType::Api if auth == maestro_core::types::AuthKind::OAuth => {
            ...
            let key_ref = if let Some(kref) = oauth_key_ref {
                ...
                let kref = providers::registry::key_ref_for(&id);
                maestro_core::secrets::set_secret(&kref, key)
        ...
        ProviderAction::Remove { id } => {
            ...
            if reg.remove(&id) {
                reg.save()?;
                let _ =
                    maestro_core::secrets::delete_secret(&providers::registry::key_ref_for(&id));
        ...
        ProviderAction::Test { id, model, prompt } => {
            ...
            println!("ledgered — see `maestro quota` or the TUI dashboard");
        }
...
fn quota_cmd(provider: Option<String>, json: bool) -> Result<()> {
    let ledger = maestro_core::ledger::Ledger::open_default()?;
    let reg = providers::ProviderRegistry::load()?;
    ...
    if rows.is_empty() {
        println!("(no usage recorded yet today — try `maestro provider test --id <provider>`)");
    }
    Ok(())
}
