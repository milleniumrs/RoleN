...
const SKIP_DIRS: &[&str] = &[".git", "target", "node_modules", ".rolen-overlay"];
        ...
        let (rel, op, payload, base_hash) = match &ch {
            Change::Write { rel, content } => {
                ...
                let base_hash = if base_file.exists() {
                    rolen_orchestrator::queue::hash_file(base, &rel.to_string_lossy())
                } else {
                    None
                };
...
mod tests {
    ...
    fn harvest_applies_diff_via_queue() {
        ...
        let base = std::env::temp_dir().join(format!("rolen-ovl-base-{tag}"));
