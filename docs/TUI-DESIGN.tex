...
one glance away (app bar + dashboard ticker).
  ...
  \item \textbf{Same core for TUI and CLI} — the TUI is a view; \texttt{rolen-cli} can do everything.
\end{itemize}
\section{2. Global Chrome}
\begin{center}
\input{tikz/chrome.tex}
\end{center}
...
\begin{center}
\input{tikz/projects.tex}
\end{center}

\subsection{3.3 Providers tab}
\begin{center}
\input{tikz/providers.tex}
\end{center}
...
Opened from Projects tab (\texttt{Enter}) or \texttt{rolen project open shop}.

\begin{center}
\input{tikz/project-window.tex}
\end{center}
...
previews the generated file with diff on regeneration; \textbf{Skills} opens the suggestion/install panel.
  ...
  \item Right-side tabs (splitter draggable): Overview / Chat / Files / Ledger / (+Requirements view).
\end{itemize}
...
user confirms in the rule editor.
  ...
  \item Done -> Mission Control. \texttt{rolen config doctor} summary shown.
\end{itemize}
\subsection{7.2 New project}
\begin{itemize}
  \item \texttt{Ctrl+N} -> wizard: name, dir, stack, git init.
  \item \textbf{Interview} (interrogator role, FR-6): batched forms; answers stream into REQUIREMENTS.json.
  \item RoleN drafts \textbf{REQUIREMENTS.md} -> user reviews in markdown tab -> approve -> REQUIREMENTS.json compiled.
...
Requirements view & \texttt{Markdown} viewer \\
...
Project window & satellite & Projects tab, \texttt{rolen project open} \\
