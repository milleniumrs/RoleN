...
│ [▶ Run] [⏸ Pause] [+ Task] [❓ Interview] [Requirements] [AGENTS.md] [Skills]      [⚙]  │ ← toolbar
├─ Tasks (DAG) ────────────────────┬─[Overview]─[Chat]─[Files]─[Ledger]──────────┤
│ ✅ 1 scaffolding                  │ Overview: goal, Requirements summary, progress       │
...
- Right-side tabs (splitter draggable): Overview / Chat / Files / Ledger / (+Requirements view).
...
2. **Interview** (interrogator role, FR-6): batched forms; answers stream into REQUIREMENTS.json.
3. Maestro drafts **REQUIREMENTS.md** → user reviews in markdown tab → approve → REQUIREMENTS.json compiled.
...
| Requirements view | `Markdown` viewer |
