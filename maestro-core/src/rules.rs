//! Rule engine (Requirements FR-3): YAML-canonical rules, condition evaluation and
//! quota-aware fallback-chain walking. Pure and UI-free — context collection
//! (health, quotas, ledger) lives in `maestro-providers::routing`.
...
/// Live state of one provider, collected by `maestro-providers::routing`.
...
pub fn decide(rules: &RuleSet, role: &str, ctx: &EvalContext) -> Result<Decision, RuleError> {
    ...
    if candidates.is_empty() {
        return Err(RuleError::NoRoute {
            role: role.into(),
            reason: "no rule matches this role (create one with `maestro rule add`)".into(),
        });
    }
