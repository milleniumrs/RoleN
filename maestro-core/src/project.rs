...
//! Pure/deterministic — LLM generation lives in `maestro-providers::generate`.
...
pub const PROJECT_FILE: &str = "maestro-project.yaml";
...
/// Validate a REQUIREMENTS.json file (`maestro prd validate`).
...
pub fn render_agents_md(meta: &ProjectMeta, prd: &PrdContent) -> String {
    ...
    s.push_str("> Generated by Maestro. Keep it updated when structure, conventions or workflows change.\n\n");
    s.push_str("## Project\n\n");
    s.push_str(&format!("{}\n\n", prd.overview.trim()));
    if !meta.stack.is_empty() {
        s.push_str(&format!("**Stack:** {}\n\n", meta.stack.join(", ")));
    }
    s.push_str("## Working agreements (enforced by Maestro)\n\n");
...
mod tests {
    ...
    fn prd_json_roundtrip_validates() {
        let dir = std::env::temp_dir().join(format!("maestro-prd-{}", std::process::id()));
