...
//! (`maestro-tui/src/mission_control.rs:1092`). That is the single worst
    ...
    pub fn spawn(ctx: egui::Context, interval: Duration) -> Self {
        ...
        let handle = {
            ...
            std::thread::Builder::new()
                .name("maestro-gui:poller".to_string())
...
fn worker(
    ...
    while !stop.load(Ordering::Relaxed) {
        ...
        let registry = match maestro_providers::ProviderRegistry::load() {
            Ok(r) => Some(r),
            Err(e) => {
            ...
            if let Some(reg) = registry.as_ref() {
                quota = reg
                    ...
                    .map(|p| {
                        (
                            p.id.clone(),
                            maestro_providers::routing::remaining_pct(&p.id),
                        )
                    })
                    .collect();
            }
            let root = maestro_core::config::Config::load()
...
/// (`maestro-tui/src/mission_control.rs:854`). Here it is on the poller thread
/// and only on the slow cadence.
fn scan_projects(root: &Path) -> Vec<ProjectRow> {
    maestro_core::project::list_projects(root)
        .into_iter()
        .map(|(dir, meta)| {
            let pending = meta
                .clarifications
                .iter()
                .filter(|c| !matches!(c.status, maestro_core::types::ClarificationStatus::Answered))
                .count();
            ProjectRow {
                id: meta.id,
                name: meta.name,
                ...
                stack: meta.stack,
                has_prd: dir.join("REQUIREMENTS.md").exists(),
...
fn collect(
    registry: Option<&maestro_providers::ProviderRegistry>,
    ...
    // `usage_today` measures from UTC midnight (maestro-core/src/ledger.rs:279);
