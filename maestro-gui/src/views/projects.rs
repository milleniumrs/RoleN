...
//!
//! Creation is wired up; the interview and Requirements/DAG build are not, because they
        ...
        .show(ui, |ui| {
            TableBuilder::new(ui)
                ...
                .header(22.0, |mut header| {
                    for title in ["Name", "Stack", "Requirements", "AGENTS", "Questions", "Skills"] {
                        header.col(|ui| {
                            ui.strong(title);
                        });
                    }
...
fn detail(ui: &mut egui::Ui, p: &crate::state::ProjectRow) {
    ...
    ui.code(format!("maestro project interview --name {}", p.id));
    ui.code(format!("maestro project build --name {}", p.id));
}
