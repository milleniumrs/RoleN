//! The agent loop (Requirements FR-12.1): chat with tool calls until done.
...
use maestro_core::types::{Session, SessionState};
use maestro_providers as providers;
use maestro_providers::chat::{HistMsg, ToolCall, ToolsChatRequest};
...
) -> Option<(maestro_core::types::Provider, String)> {
...
pub fn run(
    ...
    let result = loop {
        ...
        let _ = ledger.record(&maestro_core::types::LedgerEntry {
            id: format!(
                "le-{}",
                chrono::Utc::now().timestamp_nanos_opt().unwrap_or(0)
            ),
...
fn system_prompt(role: &str) -> String {
    format!(
        "You are Maestro's {role} agent working inside a sandboxed workspace.\n\
