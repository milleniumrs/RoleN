...
impl SettingsWindow {
    pub fn new() -> Self {
        ...
        let mut w = Self {
            base: ModalWindow::new("Settings", layout!("a:c,w:68,h:22"), window::Flags::None),
        ...
        w.add(label!(
            "'Question mode (interview thoroughness):',x:2,y:4,w:44"
        ));
        let mut cb = ComboBox::new(layout!("x:2,y:5,w:30"), combobox::Flags::None);
        ...
        w.add(label!(
            "'Parallelism: global cap (0 = auto CPU heuristic):',x:2,y:7,w:52"
        ));
        w.t_global_cap = w.add(textfield!("x:2,y:8,w:10"));
        w.add(label!("'per-provider cap:',x:16,y:8,w:18"));
        w.t_provider_cap = w.add(textfield!("x:34,y:8,w:10"));

        w.add(label!("'Quota alerts — warn %:',x:2,y:10,w:24"));
        w.t_warn = w.add(textfield!("x:26,y:10,w:8"));
        w.add(label!("'critical %:',x:38,y:10,w:12"));
        w.t_crit = w.add(textfield!("x:50,y:10,w:8"));

        w.add(label!(
            "'Secrets: OS keychain → age vault fallback (MAESTRO_VAULT_PASSWORD).',x:2,y:12,w:64"
        ));
        w.add(label!("'Keybindings, sandbox allow-lists and more: config.toml (maestro config path).',x:2,y:13,w:64"));

        w.l_status = w.add(label!("'',x:2,y:15,w:60"));
        w.b_save = w.add(button!("'&Save',x:22,y:17,w:12"));
        w.b_cancel = w.add(button!("'&Cancel',x:38,y:17,w:12"));
