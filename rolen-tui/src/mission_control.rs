    ...
    pub fn new() -> Self {
        ...
        w.m_project = w.appbar().add(MenuButton::new(
            ...
            menu!(
                ...
                {'&Build (Requirements/AGENTS/skills/DAG)', cmd:BuildProject},
                {-},
        ...
        t.add(1, label!("'Enter on a project = detail (Requirements/AGENTS.md/skills) · Enter on a session = transcript · Project menu: New / Interview / Build',l:1,b:0,r:1"));
        ...
        match rolen_core::project::scaffold(&draft.name, &draft.description, stack, &root) {
            Ok((meta, dir)) => {
                dialogs::message(
                    "New Project",
                    &format!(
                        "Project '{}' scaffolded at {}.\n\nNext: Project → Interview (clarification questions),\nthen Project → Build (REQUIREMENTS.md/REQUIREMENTS.json/AGENTS.md/skills/tasks.yaml).",
                        meta.id,
                        dir.display()
                    ),
                );
