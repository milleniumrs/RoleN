//! Off-thread work.
//!
//! Nothing in the RoleN workspace is async: `reqwest` is used in blocking
...
//! (`rolen_providers::routing::collect`), so anything that talks to a
...
/// One line of `rolen_core::doctor::run_all`.
#[derive(Debug, Clone)]
pub struct CheckRow {
    pub name: String,
    pub ok: bool,
...
/// save (`rolen-tui/src/settings.rs:175`), silently discarding a
/// `switch-rule` or `pause-role` setting; this does not.
#[derive(Debug, Clone)]
pub struct ConfigForm {
    pub workspace_root: String,
    pub question_mode: rolen_core::types::QuestionMode,
...
pub struct ProviderForm {
    pub id: String,
    pub ptype: rolen_core::types::ProviderType,
    pub endpoint: String,
    ...
    pub models: Vec<rolen_core::types::Model>,
}
...
pub enum JobMsg {
    ...
    ModelsDiscovered(Result<Vec<rolen_core::types::Model>, String>),
    ...
    ChatReply(Result<rolen_providers::conversation::Turn, String>),
...
pub fn run_cli_task(
    ...
    let reg = match rolen_providers::ProviderRegistry::load() {
        Ok(r) => r,
    ...
    match rolen_cliadapters::run_cli_session(&provider, &task, &workdir, None, &mut on_event) {
...
pub fn chat_turn(
    provider_id: String,
    model: String,
    history: Vec<rolen_providers::chat::ChatMessage>,
    session_id: String,
    prior: rolen_providers::conversation::Totals,
) -> JobMsg {
    match rolen_providers::conversation::send(
        &provider_id,
        &model,
        history,
        &session_id,
...
pub fn health_check_all() -> JobMsg {
    ...
    let rows = reg
        ...
        .map(|p| {
            ...
            let h = rolen_providers::client::health(p);
            HealthRow {
                provider_id: p.id.clone(),
                ok: h.ok,
...
pub fn detect_and_register() -> JobMsg {
    let found = rolen_providers::detect::detect_all();
    let mut reg = match rolen_providers::ProviderRegistry::load() {
        Ok(r) => r,
    ...
    for mut p in found {
        ...
        if p.ptype != rolen_core::types::ProviderType::Cli && p.models.is_empty() {
            if let Ok(models) = rolen_providers::client::list_models(&p) {
                p.models = models;
            }
        }
        reg.upsert(p);
...
pub fn doctor() -> JobMsg {
    let rows = rolen_core::doctor::run_all()
        .into_iter()
        .map(|c| CheckRow {
...
pub fn scaffold_project(name: String, description: String, stack: String) -> JobMsg {
    let cfg = match rolen_core::config::Config::ensure() {
        Ok((c, _created)) => c,
    ...
    let stack: Vec<String> = stack
        ...
        .filter(|s| !s.is_empty())
        .collect();
    match rolen_core::project::scaffold(&name, &description, stack, &cfg.general.workspace_root) {
        Ok((meta, dir)) => {
...
pub fn load_config() -> JobMsg {
    match rolen_core::config::Config::ensure() {
...
pub fn save_config(form: ConfigForm) -> JobMsg {
    let mut cfg = match rolen_core::config::Config::ensure() {
        Ok((c, _created)) => c,
...
fn provider_from(form: &ProviderForm) -> rolen_core::types::Provider {
    ...
    let endpoint = if endpoint.is_empty() {
        ...
        match form.ptype {
            ProviderType::OllamaLocal => Some(rolen_providers::ollama::DEFAULT_LOCAL_BASE.into()),
            ProviderType::OllamaCloud => Some(rolen_providers::ollama::DEFAULT_CLOUD_BASE.into()),
            _ => None,
        }
    } else {
...
pub fn discover_models(form: ProviderForm) -> JobMsg {
    ...
    match rolen_providers::client::list_models_with_key(&provider, key) {
...
pub fn save_provider(form: ProviderForm) -> JobMsg {
    ...
    let key = form.key.trim();
    if !key.is_empty() {
        let key_ref = rolen_providers::registry::key_ref_for(&provider.id);
        if let Err(e) = rolen_core::secrets::set_secret(&key_ref, key) {
...
pub fn dry_run(
    ...
    let rules = match rolen_core::rules::RuleSet::load() {
        Ok(r) => r,
    ...
    let ctx = match rolen_providers::routing::collect_cancellable(
        None,
        None,
        &is_cancelled,
        &mut on_provider,
    ...
    match rolen_core::rules::decide(&rules, &role, &ctx) {
        ...
        Err(rolen_core::rules::RuleError::NoRoute { role, reason }) => {
...
pub fn remove_provider(id: String) -> JobMsg {
    ...
    if let Some(kref) = key_ref {
        if rolen_core::secrets::delete_secret(&kref).is_err() {
