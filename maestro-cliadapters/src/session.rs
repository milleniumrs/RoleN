...
use maestro_core::types::{LedgerEntry, Provider, Session, SessionState};
...
pub fn run_cli_session(
    ...
    // transcript
    let transcripts = maestro_core::config::data_dir()?.join("transcripts");
    std::fs::create_dir_all(&transcripts)?;
    let transcript_path = transcripts.join(format!("{session_id}.log"));

    let prompt = format!(
        "{task}\n\n[Maestro] Work inside the current directory. Create/modify files directly; \
         your changes are reviewed and applied by the Maestro orchestrator afterwards."
    );
