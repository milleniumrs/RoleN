...
//!
//! ```text
//! cargo test -p rolen-providers --test conversation_local -- --ignored --nocapture
//! ```
//!
...
use rolen_providers::chat::{ChatMessage, ChatRequest};
use rolen_providers::conversation::{self, Totals};
...
fn a_single_message_request_cannot_answer_it() {
    let reg = rolen_providers::ProviderRegistry::load().expect("registry");
    ...
    let response = rolen_providers::client::chat(&provider, &request).expect("chat");
