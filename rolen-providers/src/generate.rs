//! LLM-driven generation for the project core (Requirements FR-5.2/FR-6.1): interview
//! questions and Requirements content, produced by the `interrogator` / `doc-writer`
...
pub fn generate_prd(meta: &ProjectMeta) -> Result<PrdContent, ProviderError> {
    ...
    let prompt = format!(
        "You are the doc-writer for a new software project. Produce the structured Requirements content.\n\
         Project name: {name}\nDescription: {desc}\nStack: {stack}\n\
