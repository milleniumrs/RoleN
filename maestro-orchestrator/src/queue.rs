//! Global write queue (Requirements FR-7.3/7.4/7.6/7.7).
//!
...
use maestro_runtime::error::RuntimeError;
use maestro_runtime::sink::{resolve_in, WriteSink};
...
mod tests {
    ...
    fn testdir(tag: &str) -> PathBuf {
        let dir = std::env::temp_dir().join(format!("maestro-queue-{tag}-{}", std::process::id()));
