//! Secret storage routing (Requirements FR-2): env var → OS keychain → encrypted vault.
//!
...
//! - FR-2.3: `ROLEN_KEY_<KEY_REF>` env vars take precedence (CI/headless).

use crate::error::CoreError;
use crate::vault;

const SERVICE: &str = "rolen";
const PROBE_KEY: &str = "__rolen_probe__";
...
/// `ROLEN_KEY_<KEY_REF>` with non-alphanumerics replaced by `_`, uppercased.
...
/// keychain is absent or would block: `ROLEN_SECRETS_BACKEND=vault`.
pub const BACKEND_ENV: &str = "ROLEN_SECRETS_BACKEND";
...
mod tests {
    ...
    fn env_var_names_are_sanitized() {
        assert_eq!(env_var_name("provider-kimi"), "ROLEN_KEY_PROVIDER_KIMI");
        assert_eq!(env_var_name("provider:kimi"), "ROLEN_KEY_PROVIDER_KIMI");
        assert_eq!(env_var_name("a.b c"), "ROLEN_KEY_A_B_C");
    }
}
